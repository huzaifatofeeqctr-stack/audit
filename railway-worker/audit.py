...
def gather(opp):
    ...
    contracts = account_contracts(opp["AccountId"])
    ...
    bundle = {
        ...
        "account": acct,
        "line_items": [{k: li.get(k) for k in ["Package_Type__c", "Platform_Fee__c",
                        "Number_Of_Months__c", "UnitPrice"]} | {"Product": li["Product2"]["Name"]}
                       for li in line_items(opp["Id"])],
...
def audit_message(alert_text, channel_id):
